...
FingerSetup::FingerSetup(QObject *parent)
    ...
    m_state{FingerSetup::FingerSetupCommand::Idle}, m_step{0},
    m_commandReturnStatus{FingerSetup::FingerCommandReturnStatus::IDLE},
    m_fingerID{-1}, m_isConnected{false} {
    m_port = new QSerialPort(this);
    connect(m_port, &QSerialPort::readyRead, this, &FingerSetup::onReadyRead,
            Qt::QueuedConnection);
    connect(m_port, &QSerialPort::errorOccurred, this, &FingerSetup::onError,
            Qt::QueuedConnection);
    connect(this, &FingerSetup::fingerIDChanged, &m_loop, &QEventLoop::quit,
            Qt::QueuedConnection);
}
...
void FingerSetup::sendData(const QByteArray &data) {
    ...
    m_commandReturnStatus =
        FingerSetup::FingerCommandReturnStatus::IDLE; // Reset status after
        // sending data
}
...
void FingerSetup::onReadyRead() {
    ...
    if (buffer.size() < 12) {
        return; // Wait for more data to complete the packet
    }
...
void FingerSetup::dataProcess(const QByteArray &data) {
    ...
    case COMMAND_EXECUTION_COMPLETE:
        m_commandReturnStatus = FingerSetup::FingerCommandReturnStatus::
    ...
    case 0x13:
        ...
        m_commandReturnStatus =
            FingerSetup::FingerCommandReturnStatus::FAIL_TO_EXECUTE_COMMAND_STATUS;
        break;
    case NO_FINGER_ON_THE_SENSOR:
        // qInfo() << "Put your finger on the sensor.";
        m_commandReturnStatus =
            FingerSetup::FingerCommandReturnStatus::NO_FINGER_ON_THE_SENSOR_STATUS;
        break;
    ...
    case FINGER_DOES_NOT_MATCH:
        ...
        m_commandReturnStatus =
            FingerSetup::FingerCommandReturnStatus::FINGER_DOES_NOT_MATCH_STATUS;
        break;
    ...
    case PAGEID_OUT_OF_RANGE:
        ...
        m_commandReturnStatus =
            FingerSetup::FingerCommandReturnStatus::PAGEID_OUT_OF_RANGE_STATUS;
        break;
    ...
    case FAIL_TO_DELETE_TEMPLATE:
        ...
        m_commandReturnStatus =
            FingerSetup::FingerCommandReturnStatus::FAIL_TO_DELETE_TEMPLATE_STATUS;
        break;
    ...
    case ERROR_WHEN_WRITING_FLASH:
        ...
        m_commandReturnStatus =
            FingerSetup::FingerCommandReturnStatus::ERROR_WHEN_WRITING_FLASH_STATUS;
        break;
    case INVALID_REGISTER_NUMBER:
        qInfo() << "Invalid register number.";
        m_commandReturnStatus =
            FingerSetup::FingerCommandReturnStatus::INVALID_REGISTER_NUMBER_STATUS;
        break;
    default:
        break;
    }
...
void FingerSetup::genImage() {
    ...
    QByteArray packet = buildPacket(0x01, content);
    this->sendData(packet);
    qInfo() << "Image generation started.";
...
void FingerSetup::img2Tz(uint8_t bufferId) {
    ...
    QByteArray packet = buildPacket(0x01, content);
    this->sendData(packet);
    qInfo() << "Image to template conversion started."
            << "Buffer ID:" << static_cast<int>(bufferId);
...
void FingerSetup::search(uint8_t bufferId, uint16_t startPage,
                         uint16_t pageNum) {
    QByteArray content;
    ...
    qInfo() << "Searching for fingerprint data started.";
}

void FingerSetup::handleConfirmation(
    FingerSetup::FingerCommandReturnStatus status) {
    if (status == FingerSetup::FingerCommandReturnStatus::
        COMMAND_EXECUTION_COMPLETE_STATUS) {
        switch (m_step) {
        case 0:
...
void FingerSetup::enroll() {
    ...
    qInfo() << "Starting fingerprint enrollment process..."
            << "Finger ID:" << m_fingerID;
    this->handleConfirmation(
        FingerSetup::FingerCommandReturnStatus::
        IDLE); // Start the enrollment process by capturing the first image
    if (!m_isConnected) {
        connect(this, &FingerSetup::confirmationCodeReceived, this,
                &FingerSetup::handleConfirmation);
        m_isConnected = true;
    }
    qInfo() << "Enrollment process initialized.";
}

void FingerSetup::fingerPrint() {
    m_step = 0; // Reset step for new enrollment
    qInfo() << "Scanning for finger print...";
    this->handleFingerPrint(
        FingerSetup::FingerCommandReturnStatus::
        IDLE); // Start the enrollment process by capturing the first image
    if (!m_isConnected) {
        connect(this, &FingerSetup::confirmationCodeReceived, this,
                &FingerSetup::handleFingerPrint);
        m_isConnected = true;
    }
    qInfo() << "Finger print process initialized.";
}

void FingerSetup::handleFingerPrint(
    FingerSetup::FingerCommandReturnStatus status) {
    if (status == FingerSetup::FingerCommandReturnStatus::
        COMMAND_EXECUTION_COMPLETE_STATUS) {
        switch (m_step) {
        case 0:
            if (m_state == FingerSetup::FingerSetupCommand::GenImage) {
                this->setMessage("Image generated, processing...");
                this->img2Tz(1);
                m_step++;
            }
            break;
        case 1:
            if (m_state == FingerSetup::FingerSetupCommand::Img2Tz) {
                this->setMessage(
                    "Image to template conversion completed, registering model...");
                this->search(1, 0, 100); // Search in the first buffer with all pages
                m_step++;
            }
            break;
        case 2:
            if (m_state == FingerSetup::FingerSetupCommand::Search) {
                qInfo() << "Found a matching finger<=========================>";
                this->fingerPrint();
                qInfo() << "Finger print process completed.";
            }
            break;
        default:
            qInfo() << "Mismatch step: " << m_step;
...
int FingerSetup::fingerID() const { return m_fingerID; }
...
QString FingerSetup::message() const { return m_message; }
